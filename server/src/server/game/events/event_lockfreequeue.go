...
// command: "go-gencon -type Event -cont LockFreeQueue -name Queue"
...
// NewQueue creates a new lock free queue of Event
func NewQueue() *Queue {
...
func (q *Queue) Iterate() <-chan *Event {
	c := make(chan *Event)
	go func() {
		for {
			item, ok := q.Dequeue()
